...
def run_doctor(args):
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`nazar doctor --ack {hit.advisory.id}`."
                )
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'nazar setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'nazar setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'nazar setup' to create one")
                issues.append("Run 'nazar setup' to create .env")
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'nazar config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.hermes/.env or run 'nazar setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'nazar setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'nazar config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'nazar setup' to migrate config")
                else:
                    issues.append("Run 'nazar doctor --fix' or 'nazar setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'nazar doctor --fix'")
        except Exception:
            pass
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'nazar doctor --fix' to checkpoint")
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    ...
                    if should_fix:
                        ...
                        issues.append(f"Broken symlink at {_cmd_link_display}/hermes — run 'nazar doctor --fix'")
                ...
                if should_fix:
                    ...
                    issues.append(f"Missing {_cmd_link_display}/hermes symlink — run 'nazar doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'nazar-agent[vercel]')",
                "Install the Vercel optional dependency: pip install 'nazar-agent[vercel]'",
                issues,
            )
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'nazar config set model.provider <provider>' "
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'nazar setup' to configure missing API keys for full tool access")
    except Exception as e:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                _fail_and_issue(
                    ...
                    "No Honcho API key — run 'nazar memory setup'",
                    issues,
                )
            else:
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'nazar doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
