...
(``nazar-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``nazar tools`` or the manual pip command.
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``nazar update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def active_features() -> list[str]:
    ...
    Used by ``nazar update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``nazar update``. Never raises; lazy-install failures
