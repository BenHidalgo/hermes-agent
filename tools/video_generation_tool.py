...
enable video_gen/<name>``) and selecting it in ``nazar tools`` → Video
Generation.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `nazar tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `nazar tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `nazar tools` → "
...
# `nazar tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`nazar tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `nazar tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
