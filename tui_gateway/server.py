...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`nazar setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`nazar gateway` is long-running — run it in another terminal"
    ...
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`nazar config edit` needs $EDITOR in a real terminal"
    return None
    ...
    try:
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `nazar model` to configure",
            )
