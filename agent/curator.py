...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``nazar update``. On first observation we seed ``last_run_at`` to "now"
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``nazar model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `nazar model` → auxiliary
